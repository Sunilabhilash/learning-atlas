...
This is not how it should be built long-term. Data and code are welded together — adding a question means editing the app. That's the exact pain that's driving the next phase.

**Where it's going:** a Python service on the Pi, questions in external JSON, an adaptive engine that tracks what she actually understands, and a weekly pipeline that turns her school's PDF into fresh questions automatically.
